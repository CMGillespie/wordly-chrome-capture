      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (m.type === "status") {
            ...
            else reject(new Error(`Wordly error ${m.code}: ${m.message || "Connection refused"}`));
          }
        } catch(e) {}
      };
    });
...
function handleWSSMessage(event) {
  try {
    ...
    // Track ALS language switches
    else if (msg.type === "status" && msg.languageCode && msg.languageCode !== state.language) {
...
function handleStop() {
  ...
  setState({ status: "idle", sessionId: null, passcode: null,
             language: null, speakerName: null, error: null });
}
