...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const saved = await chrome.storage.local.get([
    "lastSessionId", "lastPasscode", "lastLanguage", "lastSpeakerName", "lastAls", "lastVolume"
  ]);
  if (saved.lastSessionId)   document.getElementById("session-id").value   = saved.lastSessionId;
  if (saved.lastPasscode)    document.getElementById("passcode").value      = saved.lastPasscode;
...
function buildPresetButtons(activeLang) {
...
async function startCapture() {
  ...
  await chrome.storage.local.set({ lastSessionId: sessionId, lastPasscode: passcode, lastLanguage: language, lastSpeakerName: speakerName, lastAls: alsEnabled });
  buildPresetButtons(language);
